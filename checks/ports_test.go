...
func TestOpenPorts(t *testing.T) {
	tests := []struct {
		name             string
		tasklistexecutor *utils.MockCommandExecutor
		netstatexecutor  *utils.MockCommandExecutor
		want             checks.Check
	}{
		{
			name:             "no open ports",
			tasklistexecutor: &utils.MockCommandExecutor{Output: "	\r\n Image Name 	PID 	Session Name 	Session# 	Mem Usage\r\n	\r\n", Err: nil},
			netstatexecutor:  &utils.MockCommandExecutor{Output: "	\r\n Active Connections \r\n	\r\n	Proto	Local Address	Foreign Address	State\r\n", Err: nil},
		...
		{
			name:             "tasklist error",
			tasklistexecutor: &utils.MockCommandExecutor{Output: "", Err: errors.New("")},
			netstatexecutor:  &utils.MockCommandExecutor{Output: "", Err: nil},
			want:             checks.NewCheckErrorf("OpenPorts", "error running tasklist", errors.New("")),
		},
		{
			name:             "netstat error",
			tasklistexecutor: &utils.MockCommandExecutor{Output: "	\r\n Image Name 	PID 	Session Name 	Session# 	Mem Usage\r\n	\r\n", Err: nil},
			netstatexecutor:  &utils.MockCommandExecutor{Output: "", Err: errors.New("")},
			want:             checks.NewCheckErrorf("OpenPorts", "error running netstat", errors.New("")),
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := checks.OpenPorts(tt.tasklistexecutor, tt.netstatexecutor)
