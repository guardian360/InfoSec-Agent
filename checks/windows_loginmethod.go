...
func LoginMethod() Check {
	...
	for _, element := range names {
		...
		if RegistryKey.CheckKey(key, element) == "{D6886603-9D2F-4EB2-B667-1971041FA96B}" {
			result.Result = append(result.Result, "PIN")
		} else if RegistryKey.CheckKey(key, element) == "{2135F72A-90B5-4ED3-A7F1-8BB705AC276A}" {
			result.Result = append(result.Result, "Picture Logon")
		} else if RegistryKey.CheckKey(key, element) == "{60B78E88-EAD8-445C-9CFD-0B87F74EA6CD}" {
			result.Result = append(result.Result, "Password")
		} else if RegistryKey.CheckKey(key, element) == "{BEC09223-B018-416D-A0AC-523971B639F5}" {
			result.Result = append(result.Result, "Fingerprint")
		} else if RegistryKey.CheckKey(key, element) == "{8AF662BF-65A0-4D0A-A540-A338A999D36F}" {
			result.Result = append(result.Result, "Facial recognition")
		} else if RegistryKey.CheckKey(key, element) == "{27FBDB57-B613-4AF2-9D7E-4FA7A66C21AD}" {
