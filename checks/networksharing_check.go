...
func NetworkSharing(executor commandmock.CommandExecutor) Check {
	...
	if trueCounter == total && falseCounter == 0 {
		return NewCheckResult(3, 0, "Network sharing is enabled")
	}
	if trueCounter > 0 && trueCounter < total && falseCounter > 0 {
		return NewCheckResult(3, 1, "Network sharing is partially enabled")
	}
	if trueCounter == 0 && falseCounter == total {
		return NewCheckResult(3, 2, "Network sharing is disabled")
	}
	return NewCheckResult(3, 3, "Network sharing status is unknown")
}
