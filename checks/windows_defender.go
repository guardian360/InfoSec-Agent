...
func WindowsDefender() Check {
	...
	windowsDefenderKey, err := RegistryKey.OpenRegistryKey(RegistryKey.NewRegistryKeyWrapper(registry.LOCAL_MACHINE), `SOFTWARE\Microsoft\Windows Defender`)
	if err != nil {
		return NewCheckErrorf("WindowsDefender", "error opening registry key", err)
	}
	// Close the key after we have received all relevant information
	defer RegistryKey.CloseRegistryKey(windowsDefenderKey)

	// Open the Windows Defender real-time protection registry key, representing the periodic scan
	realTimeKey, err := RegistryKey.OpenRegistryKey(RegistryKey.NewRegistryKeyWrapper(registry.LOCAL_MACHINE),
		`SOFTWARE\Microsoft\Windows Defender\Real-Time Protection`)
	if err != nil {
		return NewCheckErrorf("WindowsDefender", "error opening registry key", err)
	}
	// Close the key after we have received all relevant information
	defer RegistryKey.CloseRegistryKey(realTimeKey)
