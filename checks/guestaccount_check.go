...
func GuestAccount(
	...
	if err != nil {
		return NewCheckErrorf(2, "error executing command Get-WmiObject", err)
	}
	...
	if !found {
		return NewCheckResult(2, 0, "Guest localgroup not found")
	}
	...
	if err != nil {
		return NewCheckErrorf(2, "error executing command net localgroup", err)
	}
	outputString = strings.Split(string(output), "\r\n")
	guestUser := ""
	...
	if guestUser == "" {
		return NewCheckResult(2, 0, "Guest account not found")
	}

	// Retrieve current username
	currentUser, err := utils.CurrentUsername()
	if err != nil {
		return NewCheckErrorf(2, "error retrieving current username", err)
	}

	// Retrieve the word for 'yes' from the currentUser language
	output, err = executorYesWord.Execute("net", "user", currentUser)
	if err != nil {
		return NewCheckErrorf(2, "error executing command net user", err)
	}
	outputString = strings.Split(string(output), "\r\n")
	...
	if strings.Contains(outputString[5], yesWord) {
		return NewCheckResult(2, 1,
			"Guest account is active")
	}
	return NewCheckResult(2, 2, "Guest account is not active")
}
