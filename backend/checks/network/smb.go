...
//
// The function works by executing the commands to check the status of SMB1 and SMB2 protocols using the provided executors.
// It then parses the output of the commands to determine whether the protocols are enabled or not.
func SmbCheck(smbexecutor mocking.CommandExecutor) checks.Check {
	var resultID int
	smb1, smb2, resultID, err := SmbEnabled(smbexecutor, resultID)

	if err != nil {
		return checks.NewCheckError(checks.SmbID, err)
	}

	return checks.NewCheckResult(checks.SmbID, resultID, smb1, smb2)
}
...
//
// The function works by executing a PowerShell command to get the server configuration of the specified SMB protocol.
// It then parses the output of the command to determine whether the protocol is enabled or not.
